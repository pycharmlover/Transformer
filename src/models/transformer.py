"""
Encoder-Decoder Transformer 模型（使用T5风格的相对位置编码）

包含:
- token embedding（可选择共享 embedding）
- encoder / decoder（内部使用相对位置偏置）
- 输出投影到 vocab size（用于交叉熵损失）

注意: 
1. 使用相对位置编码，不在embedding层添加位置信息
2. 位置信息通过注意力层的相对位置偏置来体现
"""
...
class TransformerSeq2Seq(nn.Module):
    def __init__(self, vocab_size: int, d_model: int = 512, num_layers: int = 6, 
        ...
        Args:
            ...
            attention_type: 注意力机制类型
                - "standard": 标准注意力 O(n^2)（支持T5相对位置编码）
        ...
        # token embeddings（不添加位置编码，位置信息通过相对位置偏置体现）
    ...
    def forward(self,
        ...
        """
        # Embedding + scale
        # T5风格：只使用token embedding，不添加位置编码
        # 位置信息通过注意力层的相对位置偏置来体现
